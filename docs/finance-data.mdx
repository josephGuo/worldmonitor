...
The Markets panel supports a user-customizable watchlist, letting analysts track specific instruments beyond the default index set. Tickers are added through a search-first picker in the Watchlist modal: typing a ticker or company name queries `/api/symbol-search` (a cached Finnhub search wrapper) and the analyst picks from resolved results, so every stored entry is a symbol the pipeline recognizes.
...
Watchlist state is stored in `localStorage` as `wm-market-watchlist-v1`, deduplicated by symbol and capped at **50 custom entries**. The cap bounds the custom picks only — they are appended to the default universe, which is already larger than 50. Changes emit a `wm-market-watchlist-changed` CustomEvent for live synchronization across panels without a page reload.
